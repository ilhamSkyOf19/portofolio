...
const CardPortofolio: FC<Props> = ({ id, shape, type, title, handleActive }) => {
    return (
        <div className={clsx(
            'w-full h-[25rem] rounded-2xl  flex flex-col justify-start items-center px-5 pt-6 relative transition-all duration-300 ease-in-out',
            ...
            <div className='flex-1 w-full flex flex-col justify-start items-start gap-3 pt-5 cursor-pointer' >
                ...
                {/* title */}
                <h2 className={clsx(
                    "text-lg font-semibold transition-all duration-300 ease-in-out text-text-primary",

                )}>
                    {title}
                    <span className="inline-flex items-center ml-1 relative">
                        <MdArrowOutward className={clsx(
                            "text-primary text-2xl ml-1 origin-bottom-left  transition-transform duration-300 ease-in-out absolute -top-4.5",
                        )} />
                    </span>
                </h2>
