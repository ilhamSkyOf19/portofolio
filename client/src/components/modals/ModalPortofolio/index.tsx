...
const ModalPortofolio: FC<Props> = ({
  id,
  refComponent,
  active,
  setActive,
  ...
  return (
    ...
    >
      <div className="w-full flex flex-col justify-start items-center animate-fade-in">
        <div
          ref={refComponent as RefObject<HTMLDivElement>}
          className="bg-tertiary-dark w-full min-h-[100vh] rounded-2xl pt-4 pb-12 px-4.5 relative shadow-2xl shadow-[rgba(255,_255,_255,_0.05)] md:px-14 lg:px-10"
        >
          {/* button close */}
          ...
          <div className="w-full grid grid-cols-1 lg:grid-cols-2 lg:gap-6">
            <div className="bg-black w-full h-[15rem] rounded-2xl relative md:h-[30rem] lg:h-[25rem] ">
              {id === "1" ? (
                ...
                <ShapeTwo color={data?.color as string}>
                  <SwiperPortofolio mockup={5} end={5} />
                </ShapeTwo>
              ) : null}
            </div>
              ...
              <div className="grid gird-cols-1 md:grid-cols-2 gap-6 ">
                ...
                <ElementSpresifikasi element="design">
                  {data?.design.map((item, index) => (
                    <BoxElementPortofolio key={index} label={item} />
                  ))}
                </ElementSpresifikasi>
...
// element spesifikasi
const ElementSpresifikasi: FC<ElementSpesifikasiProps> = ({
  element,
  children,
}) => {
  return (
    <div className="w-full flex flex-col justify-start items-start gap-2">
      {/* element */}
      <p className=" capitalize font-semibold text-lg text-white">{element}</p>
      {/* description */}
